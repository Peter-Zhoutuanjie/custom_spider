...
class CustomSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if data_num > 0:
            for i in range(data_num):
                try:
                    ...
                    goods_name = response.xpath("//div/tr[%d]/div[1]/td[2]/div/@title" % i).extract()
                    if len(goods_name) < 1:
                        item['goods'] = ''
                    else:
                        item['goods'] = goods_name[0]
        ...
        CustomSpider.datasource.del_url_partner(url_obj[0])
    ...
    def get_url():
        ...
        try:
            if len(CustomSpider.url_lst) == 0:
                temp = CustomSpider.datasource.get_urls_partner()
                for obj in temp:
                    url_key = CustomSpider.url_pre + obj[1]
                    CustomSpider.url_dict[url_key] = obj
                    CustomSpider.url_lst.append(url_key)
